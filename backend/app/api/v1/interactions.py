...
@router.get(
    ...
    description=(
        ...
        "При создании карточка автоматически получает опубликованный маршрут, назначенный "
        "её группе контрагента; `workflow_id` позволяет явно выбрать маршрут той же группы."
    ),
